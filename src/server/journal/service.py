...
def create_awareness_session(
    ...
    events = [_normalize_content(item) for item in payload.objective_events]
    if payload.selected_event_index >= len(events):
        raise HTTPException(
    ...
    try:
        session = JournalAwarenessSessionDAO(db).create(
            ...
            selected_event_index=payload.selected_event_index,
            emotion_label=_normalize_content(payload.emotion_label),
            emotion_note=_normalize_content(payload.emotion_note),
            present_anchor=_normalize_content(payload.present_anchor),
...
def update_awareness_session_review(
    ...
    *,
    ...
    values = payload.model_dump(exclude_unset=True)
    if "review_comment" in values and values["review_comment"] is not None:
        values["review_comment"] = values["review_comment"].strip() or None
    if "reward_label" in values and values["reward_label"] is not None:
        values["reward_label"] = values["reward_label"].strip() or None
...
def create_resonance_item(
    ...
    *,
    ...
    default_excerpt = session.present_anchor or session.emotion_note
