...
export const GUIDED_AUDIO_BY_DAY: Record<number, GuidedAudio> = {
  ...
  4: { day: 4, label: '引导 4 准备中' },
  5: { day: 5, label: '引导 5 准备中' },
  6: { day: 6, label: '引导 6 准备中' },
  7: { day: 7, label: '引导 7 准备中' },
}
...
const TWO_WEEK_AUDIO_SEQUENCE = [1, 2, 3, 1, 2, 3, 1, 4, 5, 6, 7, 4, 5, 6]

export function getGuidedAudioDayForStreak(streakDays: number) {
  return TWO_WEEK_AUDIO_SEQUENCE[streakDays % TWO_WEEK_AUDIO_SEQUENCE.length]
}
